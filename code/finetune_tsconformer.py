...
if __name__ == "__main__":
    ...
    #print(asr_brain.modules.Transformer)

 

    # Export weights of pretrained model components and load them back to modules of ASR instance
    # This is achieved by exporting only related components/modules 
    # from speechbrain/asr-conformer-transformerlm-librispeech

    torch.save(asr_model.mods.asr_model[0].state_dict(), 'weights_file_cnn')
    asr_brain.modules.CNN.load_state_dict(torch.load('weights_file_cnn'))
    print("Exported and loaded Weights of ConvolutionFrontEnd....")

    torch.save(asr_model.mods.asr_model[1].state_dict(), 'weights_file_transformer')
    asr_brain.modules.Transformer.load_state_dict(torch.load('weights_file_transformer'))
    print("Exported and loaded Weights of Transformer module....")

    #torch.save(asr_model.mods.asr_model[1].decoder.state_dict(), 'weights_file_decoder_transformer')
    #asr_brain.modules.Transformer.decoder.load_state_dict(torch.load('weights_file_decoder_transformer'), strict=False)
    #print("Exported and loaded Weights for decoder of Transformer module....")

    torch.save(asr_model.mods.asr_model[2].state_dict(), 'weights_file_seq_lin')
    asr_brain.modules.seq_lin.load_state_dict(torch.load('weights_file_seq_lin'))
    print("Exported and loaded Weights of linear layer related to seq_lin module....")

    torch.save(asr_model.mods.asr_model[3].state_dict(), 'weights_file_ctc_lin')
    asr_brain.modules.ctc_lin.load_state_dict(torch.load('weights_file_ctc_lin'))
    print("Exported and loaded Weights of linear layer related to ctc_lin module....")

    torch.save(asr_model.mods.normalizer.state_dict(), 'weights_file_normalizer')
    asr_brain.modules.normalize.load_state_dict(torch.load('weights_file_normalizer'))
    ...
    # Freeze and unfreeze specific modules for fine tuning
    # Fine-tuning the transformer component in conformer

    for param in asr_brain.modules.CNN.parameters():
        param.requires_grad = False
    for param in asr_brain.modules.Transformer.parameters():
        param.requires_grad = False
    for param in asr_brain.modules.seq_lin.parameters():
        param.requires_grad = False
    for param in asr_brain.modules.ctc_lin.parameters():
        param.requires_grad = False
    for param in asr_brain.modules.normalize.parameters():
        param.requires_grad = False
    for param in asr_brain.modules.Transformer.encoder.parameters():
       param.requires_grad = False
    for param in asr_brain.modules.Transformer.decoder.parameters():
       param.requires_grad = True

    # Update all RelPosMHAXL blocks with LinearAttention (defined above)
    # Unfreeze LinearAttention for fine-tuning
    for i in range(12):
        asr_brain.modules.Transformer.encoder.layers[i].mha_layer = LinearAttn(dim = 512, dim_head = 16, heads = 8).cuda()
        asr_brain.modules.Transformer.encoder.layers[i].mha_layer.requires_grad = True
        asr_brain.modules.Transformer.encoder.layers[i].mha_layer.attn.requires_grad = True
